#!/usr/bin/env bun
/**
 * DNA Traits & Health Analyzer
 * Comprehensive SNP analysis from AncestryDNA raw data.
 * Runs 100% locally — no data leaves your machine.
 *
 * Usage:
 *   bun scripts/dna-traits.ts <AncestryDNA.txt>            # fun traits only
 *   bun scripts/dna-traits.ts <AncestryDNA.txt> --health   # include health SNPs
 *   bun scripts/dna-traits.ts <AncestryDNA.txt> --all      # same as --health
 */
...
const EXPERIMENTAL_SNPS: TraitSNP[] = [
...
const POLYGENIC_SCORES: PolygenicScore[] = [
  {
...
function calculatePolygenicScore(
  pg: PolygenicScore,
...
async function parseDNAFile(filepath: string): Promise<Map<string, SNPRecord>> {
...
function interpretAPOE(snps: Map<string, SNPRecord>): string | null {
...
function printSection(
  ...
  for (const trait of snpList) {
    // Skip duplicate rsIDs (e.g., rs601338 appears in both Nutrients and Immunity)
    if (seenRsids.has(trait.rsid)) continue;
    seenRsids.add(trait.rsid);

    const record = snps.get(trait.rsid);
    if (record) {
      found++;
      if (trait.category !== currentCategory) {
        currentCategory = trait.category;
        console.log(`\n  --- ${currentCategory} ---\n`);
      }
      const result = trait.interpret(record.allele1, record.allele2);
      console.log(`  ${trait.trait}`);
      console.log(`    Gene: ${trait.gene} (${trait.rsid})`);
      console.log(`    Genotype: ${record.allele1}/${record.allele2}`);
      console.log(`    > ${result}`);

      // APOE combined readout
      if (trait.rsid === 'rs7412') {
        const apoe = interpretAPOE(snps);
        if (apoe) console.log(apoe);
      }
...
const showHealth = process.argv.includes('--health') || process.argv.includes('--all');
const showExperimental = process.argv.includes('--experimental') || process.argv.includes('--all');

if (!filepath || process.argv.includes('--help')) {
  console.log(`
DNA Traits & Health Analyzer
Parses AncestryDNA (and compatible) raw data files.
Runs 100% locally — nothing leaves your machine.

Usage:
  bun scripts/dna-traits.ts <file>                 Fun traits only (${TRAIT_SNPS.length} SNPs)
  bun scripts/dna-traits.ts <file> --health         + health markers (+${HEALTH_SNPS.length} SNPs)
  bun scripts/dna-traits.ts <file> --experimental   + low-confidence psych/rare (+${EXPERIMENTAL_SNPS.length} SNPs)
  bun scripts/dna-traits.ts <file> --all            Everything above + polygenic scores

Example:
  bun scripts/dna-traits.ts ~/Downloads/AncestryDNA.txt --all

Supports: AncestryDNA, 23andMe, and other tab-delimited raw data formats.
`);
  process.exit(0);
}

const snps = await parseDNAFile(filepath);

console.log('╔═══════════════════════════════════════════╗');
console.log('║   DNA Traits & Health Analyzer            ║');
console.log('║   100% local — nothing leaves this machine║');
console.log('╚═══════════════════════════════════════════╝');
console.log(`  File: ${filepath}`);
console.log(`  SNPs loaded: ${snps.size.toLocaleString()}`);
const totalAnalyzed =
  TRAIT_SNPS.length +
  (showHealth ? HEALTH_SNPS.length : 0) +
  (showExperimental ? EXPERIMENTAL_SNPS.length : 0);
console.log(
  `  Analysis: ${totalAnalyzed} SNPs ${showHealth ? '+ polygenic scores' : '(use --all for everything)'}`
);

// --- Fun Traits ---
console.log('\n\n╔═══════════════════════════════════════════╗');
console.log('║   FUN TRAITS                              ║');
console.log('╚═══════════════════════════════════════════╝');

const traitResult = printSection('Fun Traits', TRAIT_SNPS, snps);

console.log(`  ─────────────────────────────────────────`);
console.log(`  Found ${traitResult.found}/${TRAIT_SNPS.length} trait SNPs on your chip`);
if (traitResult.missing > 0)
  console.log(`  (${traitResult.missing} not on this chip version — normal)`);
...
if (showHealth) {
  ...
  console.log('║                                           ║');
  console.log('║   ⚠  NOT medical advice.                  ║');
  console.log('║   Consumer chips miss most variants.      ║');
  console.log("║   'Negative' ≠ clear.                     ║");
  console.log('║   See a genetic counselor for real advice. ║');
  console.log('╚═══════════════════════════════════════════╝');

  const healthResult = printSection('Health', HEALTH_SNPS, snps);

  console.log(`  ─────────────────────────────────────────`);
  console.log(`  Found ${healthResult.found}/${HEALTH_SNPS.length} health SNPs on your chip`);
  if (healthResult.missing > 0)
    console.log(`  (${healthResult.missing} not on this chip version — normal)`);

  // --- Summary ---
  const total = traitResult.found + healthResult.found;
  const totalPossible = TRAIT_SNPS.length + HEALTH_SNPS.length;

  console.log('\n\n╔═══════════════════════════════════════════╗');
  console.log('║   SUMMARY                                 ║');
  console.log('╚═══════════════════════════════════════════╝');
  console.log(`  Total SNPs analyzed: ${total}/${totalPossible}`);
  console.log(`  Your chip covers:    ${snps.size.toLocaleString()} SNPs`);
  console.log(`  Human genome has:    ~4-5 million common variants`);
  console.log(`  Chip coverage:       ~${((snps.size / 4500000) * 100).toFixed(1)}%`);
  console.log();
  console.log('  Remember:');
  console.log('  • These are RISK MODIFIERS, not diagnoses');
  console.log('  • Most traits/conditions are polygenic + environmental');
  console.log('  • One SNP ≠ destiny');
  console.log('  • For anything concerning → genetic counselor');
  console.log('  • For pharmacogenomics → mention to your doctor/pharmacist');
  console.log('  • Exercise, sleep, diet, and social connection modify');
  console.log('    almost every genetic risk factor listed above');
} else {
  console.log('\n  Run with --health to include health markers.');
}

// --- Experimental ---
if (showExperimental) {
  console.log('\n\n╔═══════════════════════════════════════════╗');
  console.log('║   EXPERIMENTAL / LOW CONFIDENCE           ║');
  console.log('║                                           ║');
  console.log('║   These SNPs are real GWAS hits but:      ║');
  console.log('║   • Individual effects are TINY (1.05-1.1x)║');
  console.log('║   • Psychiatric conditions involve 100s+   ║');
  console.log('║     of genes + environment + epigenetics  ║');
  console.log('║   • One SNP tells you almost nothing      ║');
  console.log('║   • Rare disease chips miss most variants ║');
  console.log('║                                           ║');
  console.log('║   Included for completeness. Do NOT draw  ║');
  console.log('║   conclusions from these individually.    ║');
  console.log('╚═══════════════════════════════════════════╝');

  const expResult = printSection('Experimental', EXPERIMENTAL_SNPS, snps);

  console.log(`  ─────────────────────────────────────────`);
  console.log(
    `  Found ${expResult.found}/${EXPERIMENTAL_SNPS.length} experimental SNPs on your chip`
  );
  if (expResult.missing > 0)
    console.log(`  (${expResult.missing} not on this chip — especially common for rare variants)`);
}

// --- Polygenic Scores ---
if (showHealth || showExperimental) {
  console.log('\n\n╔═══════════════════════════════════════════╗');
  console.log('║   POLYGENIC SCORES (combined SNPs)        ║');
  console.log('║                                           ║');
  console.log('║   Multiple SNPs combined give a better    ║');
  console.log('║   signal than any single SNP alone.       ║');
  console.log('║   Still simplified vs. clinical PRS.      ║');
  console.log('╚═══════════════════════════════════════════╝\n');

  for (const pg of POLYGENIC_SCORES) {
    const { score, max, found } = calculatePolygenicScore(pg, snps);
    console.log(`  ${pg.name}`);
    console.log(`    ${pg.description}`);
    console.log(`    SNPs found: ${found}/${pg.snps.length} | Score: ${score}/${max}`);
    console.log(`    > ${pg.interpret(score, max, found)}`);
    console.log();
  }
}
...
if (showHealth || showExperimental) {
  const allSnpLists = [
    TRAIT_SNPS,
    ...(showHealth ? [HEALTH_SNPS] : []),
    ...(showExperimental ? [EXPERIMENTAL_SNPS] : []),
  ];
  const totalSNPs = allSnpLists.reduce((sum, list) => sum + list.length, 0);

  console.log('\n╔═══════════════════════════════════════════╗');
  console.log('║   SUMMARY                                 ║');
  console.log('╚═══════════════════════════════════════════╝');
  console.log(`  SNPs in analysis:    ${totalSNPs}`);
  console.log(`  Polygenic scores:    ${POLYGENIC_SCORES.length} combined predictions`);
  console.log(`  Your chip covers:    ${snps.size.toLocaleString()} SNPs`);
  console.log(`  Human genome has:    ~4-5 million common variants`);
  console.log(`  Chip coverage:       ~${((snps.size / 4500000) * 100).toFixed(1)}%`);
  console.log();
