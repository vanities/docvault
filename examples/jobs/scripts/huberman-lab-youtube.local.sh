...
JOB_ID="${DOCVAULT_JOB_ID:-huberman-lab-youtube-daily}"
# Huberman Lab — neuroscience & health-protocol podcast (Andrew Huberman).
# Full episodes and Essentials re-releases all land on /videos (~2-3 uploads a
# week); the channel has no separate live tab worth polling, so one source URL
# is enough.
SOURCE_URLS=(
  "https://www.youtube.com/@hubermanlab/videos"
)
DOMAIN="health"
TAGS_JSON='["huberman","hubermanlab","youtube","health"]'
...
OUT_DIR="$DATA_DIR/jobs/output/health-research/$JOB_ID/$STAMP"
LATEST_DIR="$DATA_DIR/jobs/output/health-research/$JOB_ID/latest"
