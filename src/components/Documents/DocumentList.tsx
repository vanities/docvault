...
function getGroupFromPath(filePath: string): string {
  ...
  if (lastFolder === 'other') {
    ...
    if (detected) {
      ...
      // e.g. business-docs/other/teraflop/W9.pdf → "Teraflop / Tax Forms"
    ...
    // Check if there's a client subfolder under other (e.g. other/teraflop/)
  ...
  // Show as the client name (e.g. "Teraflop", "Blueprint")
...
function getGroupFromClient(filePath: string, docType?: DocumentType): string {
  ...
  // e.g. "Teraflop_Invoice_2025-01.pdf" → "Teraflop"
  // e.g. "Art_City_1099-nec_2025.pdf" → "Art City"
              ...
              <div key={label || '__ungrouped'}>
                {showHeader && (
                  ...
                  >
                    ...
                    <span className="text-[11px] text-surface-500 font-normal">{docs.length}</span>
                  </button>
                )}
