...
function makeStore(): TimesheetStore {
  return {
    version: 1,
    clients: [{ id: 'c1', name: 'Globex Corporation', currency: 'USD', archived: false }],
    projects: [
...
describe('collectReportRows + categoryTotals', () => {
  it('filters to the window, resolves names, and totals per category', () => {
    const rows = collectReportRows(makeStore(), CFG, weekWindow('2026-07-31'));
...
describe('buildReportHtml', () => {
  it('renders totals and category sections', () => {
    ...
    const html = buildReportHtml(collectReportRows(makeStore(), CFG, window), window);
