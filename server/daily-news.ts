...
function buildSystem(
  ...
  const parts = [
    ...
    "In the Health section, give EVERY person who appears in the digest their own coverage — at least one concrete sentence each, citing their actual numbers. Do NOT let one person's story (e.g. an active illness) crowd the others out of the section; a sick household member is the lead, not the only subject. Be a supportive coach as well as a reporter: when someone's numbers are good (steps, workouts, resting heart rate, weight trend), say so plainly — one warm, specific affirmation per person is welcome. When sleep falls short (under ~7 hours total, or under ~1 hour of deep sleep), call it out directly with a gentle, actionable nudge (e.g. an earlier wind-down tonight) rather than burying it in neutral prose. Encouraging and concrete, never clinical or scolding.",
    editionType === 'weekly'
